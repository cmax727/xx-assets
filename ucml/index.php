	...
	foreach($load_modules as $load_module_key => $load_module){
		...
		if($module && module_security::is_logged_in()){
			if($module->get_page_title()){
				$page_title = $module->get_page_title() . $page_title_delim . $page_title;
			}
		}
...
if(!trim($page_title)){
    $page_title = $page_title  . ($page_title ? ' :: ' : '') . module_config::s('admin_system_name','Ultimate Client Manager');
}
