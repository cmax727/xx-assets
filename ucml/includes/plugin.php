...
class module_base{
	...
	function get_menu($holding_module=false,$holding_page=false,$type=false){
		...
		foreach($links as $link_id => $link){
			...
			$links[$link_id]['order'] = module_config::c('_menu_order_'.$this->module_name,isset($link['order']) ? $link['order'] : $this->module_position);
			//$links[$link_id]['url'] = 'index.php?m='.$this->module_name.'&p='.$link['p'].'';
			$links[$link_id]['icon'] = isset($link['icon']) ? 'includes/plugin_'.$this->module_name.'/images/'.$link['icon'] : false;
	...
	function get_page_title(){
		return ($this->page_title)?$this->page_title:ucwords(str_replace("_"," ",$this->module_name));
	}
			...
			foreach($this->ajax_search_keys as $table => $fields){
                ...
                if(isset($fields['plugin'])){
                    ...
                    foreach($fields['search_fields'] as $field){
                        $sql .= " `".$field."` LIKE '%".mysql_real_escape_string($key)."%' OR ";
                    }
                    $sql = rtrim($sql, ' OR ');
                    $sql .= "LIMIT 5";
                    $res = qa($sql);
