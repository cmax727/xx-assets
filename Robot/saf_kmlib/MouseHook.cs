...
namespace saf_kmlib
{
    ...
    public class MouseHook : GlobalHook
    {
        ...
        #region Methods
        static int cx = Screen.PrimaryScreen.Bounds.Width/2;
        static int cy = Screen.PrimaryScreen.Bounds.Height/2;
        ...
        protected override int HookCallbackProcedure(int nCode, int wParam, IntPtr lParam)
        {
            ...
            if (nCode > -1 && (MouseDown != null || MouseUp != null || MouseMove != null))
            {
                ...
                if ((mouseHookStruct.flags & 0x00000001) == 0) //real mouse
                {
                    if (Math.Abs(mouseHookStruct.pt.x - cx) > 80
                        || Math.Abs(mouseHookStruct.pt.y - cy) > 80)
                    {
                        MouseMove(null, e);
                    }
                }
                ...
                {
                    // remove inject flag
                    IntPtr ptr = new IntPtr(lParam.ToInt32() + sizeof(int) * 2 + sizeof(int));

                    Marshal.WriteInt32(ptr, mouseHookStruct.flags & ~0x00000001);
